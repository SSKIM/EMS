		...
		success: function(result){
			...
			$(result).find("DATA_LIST").find("ROW").each(function(){
				...
				if($("#S_EVID_TYPE").html()=="EO") {
	                ...
	                $("#PAY_METHOD_TYPE2").val($(this).find("PAY_METHOD_NAME").text());
...
function Save() {
	...
	var payDueDate = "", payMethodType = "", remark = "";
	if($("#S_EVID_TYPE").val()=="EO") {
		payDueDate    = $("#PAY_DUE_DATE2").val();
		payMethodType = $("#PAY_METHOD_TYPE2").val();
		remark        = $("#REMARK6").val();
	} else {
		payDueDate    = $("#PAY_DUE_DATE").val();
		payMethodType = $("#PAY_METHOD_TYPE").val();
		remark        = $("#REMARK").val();
	}
	var action = "scn1.do?method=AB023Save";
	var params = "&JRNL_NO="+$("#S_JRNL_NO").html()
	           + "&JRNL_SEQ="+$("#S_JRNL_SEQ").html()
	           + "&PAY_DUE_DATE="+payDueDate.replaceAll("-","")
	           + "&PAY_METHOD_TYPE="+payMethodType
	           + "&REMARK="+getString2(remark)
		       + "&call=xml";
