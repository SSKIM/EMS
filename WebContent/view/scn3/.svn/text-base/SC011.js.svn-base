	...
	$.ajax({type: "post", url: action, data: params, dataType: "xml", cache: false,
		success: function(result){
			...
			$(result).find("DATA_LIST").find("ROW").each(function(n){
				...
				$("#BLTN_CTGR").html($(this).find("BLTN_CTGR_NAME").text());
...
function List() {
	goLink("scn3.do?method=SC010","&screenId=SC010&PAGE_NO="+pageNo+"&FIND_KIND="+findKind+"&FIND_WORD="+findWord);
}
function Update() {
	goLink("scn3.do?method=SC012","&screenId=SC012&viewMode=E&BLTN_NO="+bltnNo+"&PAGE_NO="+pageNo+"&FIND_KIND="+findKind+"&FIND_WORD="+findWord);
}
function Delete() {
	...
	var action = "scn3.do?method=SC011Delete";
	var params = "&BLTN_NO="+bltnNo
	           + "&viewMode=&call=xml";
