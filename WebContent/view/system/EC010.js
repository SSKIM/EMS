...
$(document).ready(function(){
	...
	readonly("#AUTH_ID",true);
    // MANDATORY
	required("#AUTH_ID,#AUTH_NAME",true);
	// EVENT
	...
	$("#AUTH_ID2,#AUTH_NAME2").keyup(function() {
...
function Retrieve(key1) {
	...
	var j = 0;
	var action = "system.do?method=EC010Retrieve";
	var data   = "&AUTH_ID="+$("#AUTH_ID2").val()
               + "&AUTH_NAME="+getString("#AUTH_NAME2")
               + "&call=xml";
...
function Insert() {
	...
	$("#btnRetrieve").focus();
	//-----------------------------------------------------
    var action = "system.do?method=EC010Insert&call=xml";
	...
	$.ajax({type: "post", url: action, data: data, dataType: "xml", cache: false,
		success: function(result){
			...
			var key1 = $(result).find("DATA").find("dataPK").find("AUTH_ID").text();
			Retrieve(key1);
			message(I003);
		},
		error: function(xhr, ajaxOptions, thrownError){
...
function Update() {
	...
	$("#btnRetrieve").focus();
	//-----------------------------------------------------
    var action = "system.do?method=EC010Update&call=xml";
    var data   = $("#formData").serialize();
...
function Delete() {
    ...
    var action = "system.do?method=EC010Delete&call=xml";
    var data   = $("#formData").serialize();
...
function bindData(obj) {
	...
	if(!$("#btnNew").attr("disabled")) {
		$("#AUTH_ID").val(obj.cells[1].innerHTML);
	}
	$("#AUTH_NAME").val(obj.cells[2].innerHTML);
	$("#REMARK").val(obj.cells[3].innerHTML);
	setRadioValue("STATUS",obj.cells[4].innerHTML);
	$("#INS_DATE").html(obj.cells[5].innerHTML);
	$("#INS_USER").html(obj.cells[6].innerHTML);
	$("#UPD_DATE").html(obj.cells[7].innerHTML);
	$("#UPD_USER").html(obj.cells[8].innerHTML);
}
function Clear() {
	$('#formData').clearForm();
